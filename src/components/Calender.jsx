...
function Calender() {
  ...
  const selected_post = getPostDayAndTime(
  ...
  const [filter, setFilter] = useState({
    month: selected_post.month
      ? selected_post.month
      : getPostDayAndTime(new Date()).month,
    year: selected_post.year
      ? selected_post.year
      : getPostDayAndTime(new Date()).year,
  });
  ...
  const rowDataAM = Array.from({ length: 12 }, (_, index) => ({
    [`row${index + 1}`]: [],
  })).reduce((acc, obj) => ({ ...acc, ...obj }), {});
  const rowDataPM = Array.from({ length: 12 }, (_, index) => ({
    [`row${index + 1}`]: [],
  })).reduce((acc, obj) => ({ ...acc, ...obj }), {});
  ...
  const handlePostView = (posts) => {
    const postOfDay = scheduledPosts?.data.filter((item) =>
      posts.includes(item.id)
    );
    setPostPerDay(postOfDay);
  };

  const createPost = (item, count, posts) => {
    const { formattedTime } = getPostDayAndTime(
      item.scheduled_at,
      "YYYY-MM-DD HH:mm:ss"
    );
    return (
      <div
        className="post_cell position-relative overflow-hidden"
      ...
      >
        ...
        <p className="m-0 fs-14 text-white d-flex justify-content-between align-items-center">
          ...
          <div
            onClick={() => handlePostView(posts.map(({ id }) => id))}
            className="pointer d-flex justify-content-center align-items-center post-perday position-absolute start-0 end-0 bottom-0 py-2"
          >
  ...
  const createEmptyCell = () =>
    Array.from({ length: 7 }).map((_, index) => (
      <td key={index} className="text-center box-cell"></td>
    ));

  const createBox = (data) =>
    weekdays.map((day, dayIndex) => {
      const dayPosts = data.filter(
        (item) =>
          getPostDayAndTime(item.scheduled_at, "YYYY-MM-DD HH:mm:ss").day ===
          day
      );
  ...
  const createTimeLine = (rowData, meridian) =>
    Array.from({ length: 12 }).map((_, index) => (
      <tr key={index}>
        ...
        {rowData[`row${index + 1}`]?.length
          ? createBox(rowData[`row${index + 1}`])
          : createEmptyCell()}
      </tr>
    ));
          ...
          <div className="d-flex gap-3 align-items-center">
            <select name="month" onChange={handleChange}>
              <option value="" selected>
                {monthNames[filter.month - 1].name}
              </option>
